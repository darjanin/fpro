...
downcase s = map (\c -> toLower c) s

mostFrequent :: Int -> String -> String
mostFrequent n s = init $ foldr (\x y -> x ++ " " ++ y) "" $ map (\x -> snd x) $ sort $ take n $ sortBy sortF $ nub $ map (\w -> (count w ws, w)) ws
  where
    p = \ch -> ord ch < ord 'A'
    ws = map (\x -> downcase x) $ splitWords p s
    sortF a b = if fst a > fst b then LT else (if fst a < fst b then GT else (if snd a > snd b then LT else GT))

splitWords :: (Char -> Bool) -> String -> [String]
splitWords p [] = []
splitWords p (x:xs) = if first /= "" then [first] ++ (splitWords p (snd split)) else (splitWords p (snd split))
  where
    split = break p xs
    first = (if p x then "" else [x]) ++ fst split
